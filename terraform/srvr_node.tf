...
locals {
  srvr_cloud_init = templatefile("${path.module}/config/cloud_init_srvr.tftpl", {
...
resource "proxmox_virtual_environment_file" "srvr_cloud_init_config" {
  ...
  source_raw {
    ...
    data  = local.srvr_cloud_init
    file_name = "cloud_init_srvr_${substr(sha256(local.srvr_cloud_init), 0, 8)}.yaml"
  }
}
