variable "work_t4_gpu_cpu_cores" {
  type    = number
  default = 20
}

variable "work_t4_gpu_cpu_sockets" {
  type    = number
  default = 1
}

variable "work_t4_gpu_memory_gb_node0" {
  type    = number
  default = 123
}

# variable "work_t4_gpu_memory_gb_node1" {
#   type    = number
#   default = 123
# }

resource "random_integer" "delay_seconds_work_t4_gpu" {
  min = 181
  max = 240
...
resource "null_resource" "delay_before_vm_work_t4_gpu" {
  # Only changes if the random value changes (it won't) or srvr-node is replaced
  triggers = {
    delay_val = random_integer.delay_seconds_work_t4_gpu.result
  }

  provisioner "local-exec" {
    command = "echo Sleeping for ${random_integer.delay_seconds_work_t4_gpu.result} seconds... && sleep ${random_integer.delay_seconds_work_t4_gpu.result}"
  }
...
resource "proxmox_virtual_environment_file" "work_t4_gpu_cloud_init_config" {
  ...
  source_raw {
    ...
    data  = templatefile("${path.module}/config/cloud_init_work_t4_gpu.tftpl", {
        hostname   = var.work_t4_gpu_hostname
...
resource "proxmox_virtual_environment_vm" "work_t4_gpu_rancher_vm" {
  name      = var.work_t4_gpu_hostname
  node_name = var.node_name
  stop_on_destroy = false
  on_boot = true
  vm_id  = var.work_t4_gpu_vmid

  bios     = "ovmf"  # ✅ Required for q35
  machine  = "q35"   # ✅ Enables PCIe support
  # Increase 64-bit MMIO space for GPU passthrough and relocate MSI-X
  # metadata to BAR2 to avoid vfio "MSIX PBA outside of specified BAR" startup failures
  # without collapsing NVIDIA BAR layout seen by the guest driver.
  ...
  cpu {
    cores   = var.work_t4_gpu_cpu_cores
    sockets = var.work_t4_gpu_cpu_sockets
    type    = "host"     # ✅ Best performance
    numa    = true       # ✅ Enable NUMA for >1 socket
    flags   = ["+aes", "+pdpe1gb", "+pcid"]
    affinity = "0,4,8,12,16,20,24,28,32,36,40,44,48,52,56,60,64,68,72,76"
  }

  numa {
    device = "numa0"
    cpus   = "0-19"
    memory = var.work_t4_gpu_memory_gb_node0 * 1024  # 122 GiB in MiB
    hostnodes = "0"
    policy    = "bind"
  }
  ...
  #   device = "numa1"
  #   cpus   = "20-39"
  #   memory = var.work_t4_gpu_memory_gb_node1 * 1024  # 124 GiB in MiB
  #   hostnodes = "1"
  #   policy    = "bind"
  # }

  memory {
    # dedicated = var.work_t4_gpu_memory_gb_node0 * 1024 + var.work_t4_gpu_memory_gb_node1 * 1024  # fixed RAM allocation in MiB
    dedicated = var.work_t4_gpu_memory_gb_node0 * 1024  # fixed RAM allocation in MiB
  ...
  network_device {
    ...
    queues   = var.work_t4_gpu_cpu_cores * var.work_t4_gpu_cpu_sockets   # match the number of vCPUs you assigned, e.g. 10
  ...
  initialization {
    ...
    user_data_file_id = proxmox_virtual_environment_file.work_t4_gpu_cloud_init_config.id
  }
